...
describe('PullBillingModal', () => {
  ...
  it('shows a message when there are no saved AWS connections', async () => {
    ...
    render(<PullBillingModal companyId="company-1" provider="aws" onClose={jest.fn()} onPulled={jest.fn()} />);

    expect(await screen.findByText(/no amazon web services connection found/i)).toBeInTheDocument();
  });
  ...
  it('posts archiveFirst: true when Yes, but Archive Current View is clicked, calls onPulled as soon as the pull completes, and calls onClose on Done', async () => {
    ...
    render(<PullBillingModal companyId="company-1" provider="aws" onClose={onClose} onPulled={onPulled} />);
  ...
  it('reads connections from the AWS endpoint when the provider is aws', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce(jsonResponse({ connections: oneConnection }));
  ...
  it('reads connections from the Azure endpoint and labels itself for Azure', async () => {
    ...
    render(<PullBillingModal companyId="company-1" provider="azure" onClose={jest.fn()} onPulled={jest.fn()} />);
  ...
  it('posts to the Azure pull-billing route when the provider is azure', async () => {
    (global.fetch as jest.Mock)
      .mockResolvedValueOnce(jsonResponse({ connections: oneConnection }))
      .mockResolvedValueOnce(jsonResponse({ uploadedFileId: 'file-9', status: 'processed', rowCount: 4 }));
    ...
    expect(await screen.findByText('Pulled 4 rows.')).toBeInTheDocument();
    expect((global.fetch as jest.Mock).mock.calls[1][0]).toBe('/api/azure/pull-billing');
  });

  it('names Azure in the empty-connections message when the provider is azure', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce(jsonResponse({ connections: [] }));

    render(<PullBillingModal companyId="company-1" provider="azure" onClose={jest.fn()} onPulled={jest.fn()} />);

    expect(await screen.findByText(/no microsoft azure connection found/i)).toBeInTheDocument();
  });
});
